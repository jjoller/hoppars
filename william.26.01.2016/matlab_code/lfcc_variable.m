function [CC, f0, FBE, frames ] = lfcc_variable(frames, fs, alpha, window, R, M, N, L )
...
%   Author: Kamil Wojcicki, September 2011
    ...
    Nw = length(frames); % frame duration (samples)
    % min sample size : 2048 (2^11) -> zero padding
    if Nw < 2048
        frames = padarray(frames, 2048 - Nw);
        Nw = length(frames);
    end
