function [ CC, f0, logE, E, FBE, frames ] = mfcc( speech, fs, Tw, Ts, alpha, window, R, M, N, L )
...
if( nargin~= 10 ), help lfcc; return; end;
...
% SGr: change for uniformly space filters (LFCC)
hz2mel = @(h)(h);
mel2hz = @(h)(h);
...
% Framing in overlapping samples of size Nw with shifts of Ns samples
%frames = vec2frames( speech, Nw, Ns, 'cols', @(x)(x), false );
frames = buffer(speech, Nw, Nw-Ns, 'nodelay'); frames = frames(:,1:end-1);

% Aurora Standard: The logarithmic frame energy measure (logE) is computed after the offset
% compensation filtering and framing for each frame.
% A floor is used in the energy calculation which makes sure that the result is not less than -50.
% The floor value (lower limit for the argument of ln) is approximately 2e-22.
logE = log(sum(frames.^2) + exp(-50));

% as done in (Kinnunen 2010) 
E = 20*log10(std(frames) + eps);

%% LFCC 

% Preemphasis filtering (see Eq. (5.1) on p.73 of [1])
speech = filter( [1 -alpha], 1, speech ); % freqz([1 -alpha], 1,[],44100);

% Framing and windowing (frames as columns)
%frames = vec2frames( speech, Nw, Ns, 'cols', window, false );
frames = buffer(speech, Nw, Nw-Ns, 'nodelay'); frames = frames(:,1:end-1);
w = hamming(Nw); w = repmat(w,1,size(frames,2));
frames = frames.*w;
...
% compared with matlab DCT matrix implementation
% Note that there is a factor of /sqrt(2) that should be applied to the
% first coefficient and it wasn't (e.g. type dctmtx)
%     D = dctmtx(M); D(1,:) = D(1,:)*sqrt(2);

% Conversion of logFBEs to cepstral coefficients through DCT
CC =  DCT * log( FBE );


%% Dominant Harmonic % TODO: implement a better pitch estimation
[M,I] = max(MAG(1:K,:));
f0 = f(I);

% put the features as column vectors
CC = CC';
f0 = f0';
logE = logE';
E = E';
